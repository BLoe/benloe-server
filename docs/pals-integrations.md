# PALS external integrations — credential setup

Each integration activates the moment its env vars land in `/srv/benloe/.env`
(then `sudo /usr/local/sbin/pals-privops pm2-restart pals-api`). No env var =
the MCP server simply isn't registered; PALS runs fine without it.

| Integration | Env vars | Tier mapping |
|---|---|---|
| Yahoo Fantasy (on-box) | on by default (`PALS_MCP_YAHOO_URL=off` to disable; `PALS_MCP_YAHOO_TOKEN` if the local MCP requires auth) | reads T4 · lineup T3 · waivers/trades T2 |
| Google Workspace | `PALS_MCP_GOOGLE_URL` (+ optional `PALS_MCP_GOOGLE_TOKEN`) | reads T4 · own-calendar writes T3 · gmail send T2 |
| Plaid | `PALS_MCP_PLAID_URL` (+ token) | read-only T4 — there is no write path |
| Apple Health | `PALS_MCP_HEALTH_URL` (+ token) | ingestion T4 |
...
2. Set `PALS_MCP_GOOGLE_URL=http://127.0.0.1:<port>/mcp` in `.env`, restart pals-api.
...
2. Set `PALS_MCP_PLAID_URL`, restart. All financial data stays read-only (§3).
...
2. Set `PALS_MCP_HEALTH_URL`, restart. Daily metrics land in `health_daily`.
