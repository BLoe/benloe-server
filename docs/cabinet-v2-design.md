# Cabinet v2 — Product, UX, Feature & Brand Design (Phase 2)
...
conversations and a message log. But Cabinet is not a general chatbot you visit to
...
actions are untraceable. The thing that makes Cabinet *Cabinet* is exactly what the
chat UI hides.

**The product metaphor for v2:** Cabinet is your **chief of staff**, and this app
is your **situation room** — not a chat window. You are the principal; Cabinet is
...
`Cabinet` reads like a buddy-chatbot brand — friendly, generic, sits next to
...
1. **Cabinet** (recommended). Double meaning that fits exactly: the *cabinet*
   ...
   principal; Cabinet is your cabinet. Ownable, warm-but-capable, anti-chatbot.
   `cabinet.benloe.com`.
2. **Keep** the name Cabinet but re-base it from "pal/buddy" to
...
Infra names (`cabinet-api`, service dirs) can stay internal regardless; this is a
*product/brand* rename, decoupled from deployment. **This doc uses "Cabinet" as a
placeholder;** the recommendation is Cabinet. Final call is Ben's — it's cheap
to keep Cabinet and the whole design works either way.
...
"The system" = Cabinet acting on its own.
...
### Epic I — Platform engineering (Cabinet operates the server)
...
│          │                                                  │
├──────────┴─────────────────────────────────────────────────┤
│  ▸ agent presence strip (what Cabinet is doing right now)      │
└────────────────────────────────────────────────────────────┘
```
...
4. **Brain.** What Cabinet knows about you: curated memory (editable), facts,
...
- **Agent presence strip.** A persistent, quiet strip showing what Cabinet is doing
...
- **Ambient line** — "Cabinet also did 3 things overnight →" links to Ops.
...
1. **Name:** Cabinet (recommended), keep Cabinet, or another? Drives the brand.
