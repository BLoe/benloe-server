# PALS acceptance report (build §15 step 13)

Built end-to-end by the /loop run on 2026-07-07. Live at **https://pals.benloe.com**
(pals-api on 127.0.0.1:3008, PM2, running as `claude-worker`, subscription auth).

## Results

| # | Criterion | Result | Evidence |
|---|---|---|---|
| 1 | Auth wall — non-owner gets 403 | ✅ | Live: anonymous `GET https://pals.benloe.com/api/threads` → 401; gateway suite proves owner-email → 200, other artanis user → 403, bad cookie → 401 |
...
| 9 | Restore-from-backup drill | ✅ | `.backup()` of live pals.db → integrity_check **ok**, 35 tables |
...
1. **Live authenticated chat turn** — verifying a real LLM turn over HTTPS needs an owner
   ...
   **To finish:** log in at https://pals.benloe.com and send one message.
...
2. **artanis unix-hardening gap** — the red-team found `claude-worker` can *write* to
   ...
   ```
   Same reasoning applies to `apps/pals/server` (self-modification): Tier-0 blocked, but
   ...
   `apps/pals/server` to root would fully close it but breaks build-as-claude-worker;
