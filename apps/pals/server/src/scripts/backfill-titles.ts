...
async function main(): Promise<void> {
  ...
  const dataDir = process.env.PALS_DATA_DIR ?? '/srv/benloe/data/pals';
  const pals = openDb(join(dataDir, 'pals.db'));
  const update = pals.db.prepare("UPDATE thread SET title = ? WHERE id = ? AND (title IS NULL OR title = '')");

  const candidates = findCandidates(pals.db);
