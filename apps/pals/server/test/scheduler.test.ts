...
describe('jobs', () => {
  ...
  beforeEach(() => {
    dir = mkdtempSync(join(tmpdir(), 'pals-jobs-'));
  ...
  it('heartbeatFindings: empty db → none; seeded db → specific findings', () => {
    expect(heartbeatFindings(pals.db, '2026-07-07')).toEqual([]);
    updatePantry(pals.db, { name: 'chicken', location: 'fridge', quantity: 1, expires_on: '2026-07-08' });
    logMedication(pals.db, { name: 'VitD', days_supply: 10, last_filled_on: '2026-07-01' });
    upsertTask(pals.db, { title: 'file taxes', due_on: '2026-07-06' });
    const findings = heartbeatFindings(pals.db, '2026-07-07');
  ...
  it('heartbeat with no findings never calls the model', async () => {
    ...
    const audit = pals.db.prepare("SELECT decision FROM action_audit WHERE tool='heartbeat'").get() as { decision: string };
    expect(audit.decision).toBe('HEARTBEAT_OK');
  });

  it('heartbeat with findings runs a heartbeat-kind turn', async () => {
    upsertTask(pals.db, { title: 'due thing', due_on: '2000-01-01' });
  ...
  it(
    ...
    async () => {
      addJournal(pals.db, 'a pending journal entry about quokkas');
      pals.db
      ...
      expect((pals.db.prepare('SELECT embedded FROM journal_entry').get() as { embedded: number }).embedded).toBe(1);
    },
    MODEL_TIMEOUT,
  );
});
