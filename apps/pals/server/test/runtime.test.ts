...
describe('prompt assembly', () => {
  ...
  beforeEach(() => {
    dir = mkdtempSync(join(tmpdir(), 'pals-prompt-'));
...
describe('configureAuth (§9.1)', () => {
  it('subscription mode strips the API key and isolates config dir', () => {
    ...
    expect(env.CLAUDE_CONFIG_DIR).toBe('/home/claude-worker/.pals-claude');
  });

  it('api mode strips the oauth token and respects an existing config dir', () => {
    const env: Record<string, string | undefined> = {
      PALS_CLAUDE_AUTH: 'api',
      ANTHROPIC_API_KEY: 'sk-x',
      CLAUDE_CODE_OAUTH_TOKEN: 'sk-ant-oat01-y',
...
describe('AgentRuntime.run (fake SDK)', () => {
  ...
  beforeEach(() => {
    dir = mkdtempSync(join(tmpdir(), 'pals-rt-'));
    pals = openDb(join(dir, 'pals.db'));
    mem = new MemoryStore(join(dir, 'memory'));
    mem.ensureTemplates();
    pals.db.prepare("INSERT INTO thread (id, kind) VALUES ('t1','user')").run();
  });
  ...
  it('translates the stream, persists the session id, and records usage', async () => {
    ...
    // session persisted
    const row = pals.db.prepare("SELECT sdk_session_id FROM thread WHERE id='t1'").get() as any;
    expect(row.sdk_session_id).toBe('sess-123');
    // usage recorded
    const usage = pals.db.prepare('SELECT model, input_tokens, cost_usd FROM token_usage').get() as any;
  ...
  it('resumes user turns with the stored session id, not scheduled turns', async () => {
    pals.db.prepare("UPDATE thread SET sdk_session_id = 'prev-sess' WHERE id='t1'").run();
  ...
  it('falls back to opus when fable refuses', async () => {
    pals.db.prepare("UPDATE thread SET model_override = 'fable' WHERE id='t1'").run();
