...
beforeEach(() => {
  dir = mkdtempSync(join(tmpdir(), 'pals-gw-'));
...
describe('chat stream', () => {
  it('streams the turn, persists user + folded assistant messages', async () => {
    ...
    const rows = pals.db.prepare('SELECT role, parts FROM message WHERE thread_id = ? ORDER BY created_at').all(id) as { role: string; parts: string }[];
