...
let dir: string; let pals: PalsDb; let server: Server; let base: string;
...
async function start() {
  ...
  const app = buildApp({ db: pals.db, runtime: runtime as never, approvals: new ApprovalQueue(pals.db), widgetBus: new EventEmitter(), ownerEmail: OWNER, authFetch: fakeAuthFetch });
...
beforeEach(async () => { dir = mkdtempSync(join(tmpdir(), 'pals-attr-')); pals = openDb(join(dir, 'pals.db')); await start(); });
...
describe('identity attribution', () => {
  it('stamps created_by on a thread the principal opens', async () => {
    ...
    const row = pals.db.prepare('SELECT created_by FROM thread WHERE id = ?').get(id) as { created_by: string };
  ...
  it('stamps a user message with its author and tells the runtime who it is (agent)', async () => {
    ...
    const msg = pals.db.prepare("SELECT author FROM message WHERE thread_id = ? AND role = 'user'").get(id) as { author: string };
