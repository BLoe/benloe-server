...
describe('classification table (§6)', () => {
  it.each<[string, Record<string, unknown>, number]>([
    ...
    ['Bash', { command: 'sudo /usr/local/sbin/pals-privops pm2-list' }, 4],
    ...
    ['Write', { file_path: '/srv/benloe/apps/pals/web/src/App.tsx' }, 3], // web ok; server is not
    ['Bash', { command: 'npm run build' }, 3],
    ['Bash', { command: 'git add -A && git commit -m "x"' }, 3],
    ['Bash', { command: 'sudo /usr/local/sbin/pals-privops pm2-restart dada-api' }, 3],
    ['Bash', { command: 'curl -s http://localhost:3004/api/health' }, 3],
    // Tier 2 — approve-before
    ['Bash', { command: 'git push origin main' }, 2],
    ['Write', { file_path: '/srv/benloe/infra/caddy/pals.benloe.com' }, 2],
    ['Bash', { command: 'sudo /usr/local/sbin/pals-privops pm2-start /srv/benloe/apps/newapp/ecosystem.config.js' }, 2],
    ['Bash', { command: 'sudo /usr/local/sbin/pals-privops caddy-reload' }, 2],
    ...
    ['Edit', { file_path: '/srv/benloe/apps/pals/server/src/tiers/gate.ts' }, 0], // self-modification
    ['Write', { file_path: '/etc/passwd' }, 0],
    ['Write', { file_path: '/srv/benloe/infra/systemd/pals.service' }, 0],
  ...
  it('mcp catch-alls: pals tools are 4, unknown MCP is 2, yahoo reads are 4, lineup is 3', () => {
    expect(classifyToolUse('mcp__pals__log_food', {}).tier).toBe(4);
...
describe('bash classifier hardening', () => {
  ...
  it('symlinks cannot dodge protected-path checks', () => {
    const dir = mkdtempSync(join(tmpdir(), 'pals-link-'));
...
describe('gate + approvals', () => {
  ...
  beforeEach(() => {
    dir = mkdtempSync(join(tmpdir(), 'pals-gate-'));
  ...
  const auditRows = () => pals.db.prepare('SELECT tool, tier, decision FROM action_audit ORDER BY id').all() as {
    tool: string;
    tier: number;
    decision: string;
  }[];
  ...
  it('tier 4 allows and audits', async () => {
    const gate = buildGate({ db: pals.db, approvals });
  ...
  it('tier 3 allows, audits, and notifies', async () => {
    let notified = '';
    const gate = buildGate({ db: pals.db, approvals, events: { onNotify: (t) => (notified = t) } });
  ...
  it('tier 2 expires to deny', async () => {
    const shortQueue = new ApprovalQueue(pals.db, 80);
    const gate = buildGate({ db: pals.db, approvals: shortQueue });
    const r = await gate('Bash', { command: 'git push origin main' }, ctx);
    expect(r).toMatchObject({ behavior: 'deny' });
    const row = pals.db.prepare('SELECT status FROM approval').get() as { status: string };
