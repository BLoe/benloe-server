...
export const DEFAULT_POLICY: TierPolicy = {
  denyAlways: [
    ...
    '/home/claude-worker/.cabinet-claude',
    '/etc/sudoers',
    '/etc/sudoers.d',
    '/usr/local/sbin/cabinet-privops',
  ],
  denyWrite: [
    '/srv/benloe/apps/artanis',
    '/srv/benloe/apps/cabinet/server',
    '/srv/benloe/infra/systemd',
    '/srv/benloe/infra/scripts/cabinet-privops.sh',
    '/etc',
    '/usr',
    '/var',
    '/boot',
  ...
  writeAllow: ['/srv/benloe/apps', '/srv/benloe/static', '/srv/benloe/docs', '/srv/benloe/data/cabinet', '/tmp'],
...
function classifyBashPart(part: string, policy: TierPolicy): Classification {
  ...
  if (bin === 'sudo') {
    if (words[1] === '/usr/local/sbin/cabinet-privops' || words[1] === 'cabinet-privops') {
      const sub = words[2] ?? '';
...
const MCP_TIERS: [RegExp, Tier, string][] = [
  [/^mcp__cabinet__enqueue_approval$/, 4, 'cabinet-internal'],
  [/^mcp__cabinet__/, 4, 'cabinet-tool'],
