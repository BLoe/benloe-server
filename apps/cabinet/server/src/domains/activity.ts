...
// isTrainingDay (build 3) was removed in build 4 — Ben's actual routine
// (heavy lifts 2x/week, cardio every day) collapsed the training/rest
// calorie binary it fed, and the nutrition model simplified to one calorie
// target (see goalTarget, gateway/surfaces.ts). The activity-plan substrate
// above is unaffected — this only removes the now-unused day-type→calorie
// signal, not activity planning itself.
